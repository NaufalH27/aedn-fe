...
import { currencyList, type CurrencyCode } from "../../helper/currency";
import { extractKey, getFileExtension } from "../../helper/picture";
...
export default function CommissionForm({
  ...
  const [form, setForm] = useState<CommissionFormState>(data);
  ...
  const setValue = <K extends keyof CommissionFormState>(
    name: K,
  ...
  const handleEdit = async (e: React.SubmitEvent) => {
    ...
    if (!form.id) {
      setError("Oops, it seems like the commission can't be modified for now");
      setStatus("error");
      return;
    }
    ...
    try {
      ...
      showToast("success", "Edit Commission Success");
      onSuccess?.();
    } catch (err) {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      showToast("success", "Create Commission Success");
      onSuccess?.();
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unknown error";
        ...
        <div className="grid gap-5 grid-cols-[1fr_5fr]">
          <div className={underline}>
            <select
              ...
              onChange={(e) =>
                setValue("currencyCode", e.target.value as CurrencyCode)
              }
              className="w-full py-3 outline-none bg-transparent"
            >
        ...
        <div className="relative">
          ...
          {!showSuggestion &&
            ...
            !filteredCategories.some(
              (item) =>
                item.toLowerCase() === categoryInput.trim().toLowerCase()
            ) && <p className="mt-2 text-sm text-red-500">* New Category</p>}
